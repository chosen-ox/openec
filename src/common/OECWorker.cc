...
OECWorker::OECWorker(Config *conf) : _conf(conf) {
  ...
  _underfs = FSUtil::createFS(_conf->_fsType, _conf->_fsFactory[_conf->_fsType],
                              _conf);

  // tune performance
  FSObjOutputStream *tuneobjout =
      new FSObjOutputStream(_conf, "/tmptuneoecout", _underfs, 0);
  delete tuneobjout;
  FSObjInputStream *tuneobjin =
      new FSObjInputStream(_conf, "/tmptuneoecout", _underfs);
  delete tuneobjin;
}
...
void OECWorker::doProcess() {
  ...
  while (true) {
    ...
    if (rReply->type == REDIS_REPLY_NIL) {
      ...
      cout << "OECWorker::doProcess() receive a request of type " << type
           << endl;
      // agCmd->dump();
      switch (type) {
      case 0:
      ...
      //      cout << "OECWorker::doProcess().duration = " <<
      //      RedisUtil::duration(time1, time2) << endl;
      // delete agCmd
      delete agCmd;
    }
    // free reply object
...
void OECWorker::onlineWrite(string filename, string ecid, int filesizeMB) {
  ...
  // 0. send request to coordinator that I want to write a file with online
  // erasure coding
  ...
  redisFree(waitCtx);
  gettimeofday(&time2, NULL);
  cout << "OECWorker::onlineWrite.registerFile.duraiton = "
       << RedisUtil::duration(time1, time2) << endl;

  // 2. create threads for Load tasks to load data from local redis
  BlockingQueue<OECDataPacket *> **loadQueue =
      (BlockingQueue<OECDataPacket *> **)calloc(
          eck, sizeof(BlockingQueue<OECDataPacket *> *));
  for (int i = 0; i < eck; i++) {
  ...
  FSObjOutputStream **objstreams =
      (FSObjOutputStream **)calloc(ecn, sizeof(FSObjOutputStream *));
  ...
  if (finish) {
    ...
    int tmpval = htonl(1);
    rReply = (redisReply *)redisCommand(waitCtx, "rpush %s %b", wkey.c_str(),
                                        (char *)&tmpval, sizeof(tmpval));
    freeReplyObject(rReply);
    redisFree(waitCtx);
  }
  gettimeofday(&time3, NULL);
  cout << "OECWorker::onlineWrite.duration: "
       << RedisUtil::duration(time1, time3) << endl;
...
void OECWorker::offlineWrite(string filename, string ecpoolid, int filesizeMB) {
  ...
  // 0. send request to coordinator that I want to write a file with offline
  // erasure coding
  //    wait for responses from coordinator with a set of tasks
  gettimeofday(&time1, NULL);
  CoorCommand *coorCmd = new CoorCommand();
  ...
  delete agCmd;
  gettimeofday(&time2, NULL);
  cout << "offlineWrite::get response from coordinator: "
       << RedisUtil::duration(time1, time2) << endl;
  cout << "offlineWrite::objnum = " << objnum << ", basesizeMB = " << basesizeMB
       << endl;
  ...
  FSObjOutputStream **objstreams =
      (FSObjOutputStream **)calloc(objnum, sizeof(FSObjOutputStream *));
  ...
  BlockingQueue<OECDataPacket *> **loadQueue =
      (BlockingQueue<OECDataPacket *> **)calloc(
          objnum, sizeof(BlockingQueue<OECDataPacket *> *));
  for (int i = 0; i < objnum; i++) {
...
void OECWorker::loadWorker(BlockingQueue<OECDataPacket *> *readQueue,
                           string keybase, int startid, int step, int round,
                           bool zeropadding) {
  ...
  redisFree(readCtx);
  gettimeofday(&time2, NULL);
  cout << "OECWorker::loadWorker.from client.duration = "
       << RedisUtil::duration(time1, time2) << endl;
}

void OECWorker::computeWorkerDegradedOffline(
    FSObjInputStream **readStreams, vector<int> idlist,
    unordered_map<int, vector<int>> sid2Cids,
    BlockingQueue<OECDataPacket *> *writeQueue, int lostidx,
    vector<ECTask *> computeTasks, int stripenum, int ecn, int eck, int ecw) {
  // In this method, we read available data from readStreams, whose stripeidx is
  // in idlist Then we perform compute task one by one in computeTakss for each
  // stripe Finally, we put pkt for lostidx in writeQueue
...
void OECWorker::computeWorker(FSObjInputStream **readStreams,
                              ...
                              vector<ECTask *> computeTasks, int stripenum,
                              int ecn, int eck, int ecw) {
  cout << "OECWorker::computeWorker.stripenum: " << stripenum << endl;
  // In this method, we read available data from readStreams, whose stripeidx is
  // in idlist Then we perform compute task one by on in computeTasks for each
  // stripe. Finally, we put original eck data pkts in writeQueue
  OECDataPacket **curStripe =
      (OECDataPacket **)calloc(ecn, sizeof(OECDataPacket *));
  for (int i = 0; i < ecn; i++)
    curStripe[i] = NULL;
  int splitsize = _conf->_pktSize / ecw;
  for (int stripeid = 0; stripeid < stripenum; stripeid++) {
    ...
    // now computation is finished, we take out pkt from stripe and put into
    // outputstream
...
void OECWorker::computeWorker(vector<ECTask *> computeTasks,
                              ...
                              FSObjOutputStream **objstreams, int stripenum,
                              int ecn, int eck, int ecw, string ecid) {
  struct timeval time1, time2, time3;
  gettimeofday(&time1, NULL);
  cout << "OECWorker::computeWorker.stripenum: " << stripenum;
  // In this method, we fetch eck pkts from readQueue and cache into runtime
  // memory If there is need, we need to divide pkt into splits for
  // sub-packetization Then we perform calculations inside ECTask list one by
  // one, results are cached in runtime memory Finally we put data from ecn into
  // output streams
  ...
  cout << ", pktsize: " << pktsize << ", splitsize: " << splitsize
       << ", ecn: " << ecn << ", eck: " << eck << ", ecw: " << ecw
       << ", ecid:" << ecid << endl;
        ...
        if (col * row >= 1) {
          ...
          for (auto it : coefMap) {
            ...
            if (bufMap.find(target) == bufMap.end()) {
              ...
              codebuf = bufMap[target];
              if (stripeid == 0)
                cout << "code[" << codeBufIdx << "] = bufMap[" << target << "]"
                     << endl;
            }
            code[codeBufIdx] = codebuf;
  ...
  free(curStripe);
  gettimeofday(&time2, NULL);
  cout << "OECWorker::computeWorker.duration = "
       << RedisUtil::duration(time1, time2) << endl;
}
...
void OECWorker::readDisk(AGCommand *agcmd) {
  ...
  if (w == 1 || w == cidlist.size()) {
    ...
    // random read
    thread readThread =
        thread([=] { objstream->readObj(w, cidlist, slicesize); });
    BlockingQueue<OECDataPacket *> *readQueue = objstream->getQueue();
    // cacheThrad
    thread cacheThread = thread([=] {
...
void OECWorker::selectCacheWorker(BlockingQueue<OECDataPacket *> *cacheQueue,
                                  int pktnum, string keybase, int w,
                                  vector<int> idxlist,
  ...
  gettimeofday(&time2, NULL);
  cout << "OECWorker::selectCacheWorker.duration: "
       << RedisUtil::duration(time1, time2) << " for " << keybase << endl;
  redisFree(writeCtx);
}
...
void OECWorker::fetchCompute(AGCommand *agcmd) {
  ...
  // create fetch queue
  BlockingQueue<OECDataPacket *> **fetchQueue =
      (BlockingQueue<OECDataPacket *> **)calloc(
          nprevs, sizeof(BlockingQueue<OECDataPacket *> *));
  ...
  BlockingQueue<OECDataPacket *> **writeQueue =
      (BlockingQueue<OECDataPacket *> **)calloc(
          coefs.size(), sizeof(BlockingQueue<OECDataPacket *> *));
  ...
  for (int i = 0; i < nprevs; i++) {
    ...
    fetchThreads[i] =
        thread([=] { fetchWorker(fetchQueue[i], keybase, prevlocs[i], num); });
  }

  // create compute thread
  thread computeThread = thread([=] {
    computeWorker(fetchQueue, nprevs, num, coefs, computefor, writeQueue,
                  _conf->_pktSize / w);
  });
  ...
  for (int i = 0; i < computefor.size(); i++) {
    ...
    cacheThreads[i] =
        thread([=] { cacheWorker(writeQueue[i], keybase, num, r); });
  }
...
void OECWorker::fetchWorker(BlockingQueue<OECDataPacket *> *fetchQueue,
                            string keybase, unsigned int loc, int num) {
  redisReply *rReply;
  ...
  for (int i = replyid; i < num; i++) {
    ...
    // if (i == 0) cout << "OECWorker::fetchWorker.fetch first t = " <<
    // RedisUtil::duration(t1, t2) << endl;
    char *content = rReply->element[1]->str;
    OECDataPacket *pkt = new OECDataPacket(content);
    int curDataLen = pkt->getDatalen();
    fetchQueue->push(pkt);
  ...
  gettimeofday(&time2, NULL);
  cout << "OECWorker::fetchWorker.duration: "
       << RedisUtil::duration(time1, time2) << " for " << keybase << endl;
  redisFree(fetchCtx);
}
...
void OECWorker::computeWorker(BlockingQueue<OECDataPacket *> **fetchQueue,
  ...
  cout << "OECWorker::computeWorker.num: " << num << ", row: " << row
       << ", col: " << col << endl;
  cout << "-------------------" << endl;
  for (int i = 0; i < row; i++) {
    for (int j = 0; j < col; j++) {
  ...
  OECDataPacket **curstripe =
      (OECDataPacket **)calloc(row + col, sizeof(OECDataPacket *));
  char **data = (char **)calloc(col, sizeof(char *));
  char **code = (char **)calloc(row, sizeof(char *));
  while (num--) {
    // prepare data
...
void OECWorker::computeWorker(
    BlockingQueue<OECDataPacket *> **fetchQueue, int nprev,
    vector<int> prevCids, int num, unordered_map<int, vector<int>> coefs,
    vector<int> cfor,
...
void OECWorker::cacheWorker(BlockingQueue<OECDataPacket *> *writeQueue,
                            string keybase, int startidx, int num, int ref) {
  redisReply *rReply;
  ...
  gettimeofday(&time2, NULL);
  cout << "OECWorker::writeWorker.duration: "
       << RedisUtil::duration(time1, time2) << " for " << keybase << endl;
  redisFree(writeCtx);
}

void OECWorker::cacheWorker(BlockingQueue<OECDataPacket *> *writeQueue,
                            string keybase, int num, int ref) {
  struct timeval time1, time2, time3, time4;
  gettimeofday(&time1, NULL);
  ...
  gettimeofday(&time2, NULL);
  cout << "OECWorker::cacheWorker.createCtx: "
       << RedisUtil::duration(time1, time2) << endl;
  ...
  gettimeofday(&time3, NULL);
  cout << "OECWorker::cacheWorker.write all data: "
       << RedisUtil::duration(time2, time3) << endl;
  for (int i = replyid; i < count; i++) {
    redisGetReply(writeCtx, (void **)&rReply);
    freeReplyObject(rReply);
  }
  ...
  cout << "OECWorker::writeWorker.duration: "
       << RedisUtil::duration(time1, time4) << " for " << keybase << endl;
  redisFree(writeCtx);
}

void OECWorker::cacheWorker(BlockingQueue<OECDataPacket *> *writeQueue,
                            string keybase, int startidx, int step, int num,
                            int ref) {
  ...
  gettimeofday(&time4, NULL);
  cout << "OECWorker::cacheWorker6.duration: "
       << RedisUtil::duration(time1, time4) << " for " << keybase << endl;
  redisFree(writeCtx);
}
...
void OECWorker::persist(AGCommand *agcmd) {
  ...
  for (int i = 0; i < nprevs; i++) {
    string keybase = stripename + ":" + to_string(prevcids[i]);
    cout << "OECWorker::persist.fetch " << keybase << " from "
         << RedisUtil::ip2Str(prevlocs[i]) << endl;
  }
  cout << "OECWorker::persist.write as " << objname << " with " << num
       << " pkts" << endl;
  ...
  FSObjOutputStream *objstream =
      new FSObjOutputStream(_conf, objname, _underfs, num * nprevs);
  ...
  int tmpval = htonl(1);
  rReply = (redisReply *)redisCommand(writeCtx, "rpush %s %b", wkey.c_str(),
                                      (char *)&tmpval, sizeof(tmpval));
  freeReplyObject(rReply);
  redisFree(writeCtx);
...
void OECWorker::clientRead(AGCommand *agcmd) {
  ...
  metareply =
      (redisReply *)redisCommand(metaCtx, "blpop %s 0", metakey.c_str());
  ...
  rReply = (redisReply *)redisCommand(cliCtx, "rpush %s %b", skey.c_str(),
                                      (char *)&tmpval, sizeof(tmpval));
  freeReplyObject(rReply);
  redisFree(cliCtx);

  gettimeofday(&time2, NULL);
  cout << "OECWorker::clientRead.get metadata duration = "
       << RedisUtil::duration(time1, time2) << endl;
...
void OECWorker::readOffline(string filename, int filesizeMB, int objnum) {
  cout << "OECWorker::readOffline.filename: " << filename
       << ", filesizeMB: " << filesizeMB << ", objnum: " << objnum << endl;

  // create inputstream
  vector<thread> createThreads = vector<thread>(objnum);
  FSObjInputStream **objstreams =
      (FSObjInputStream **)calloc(objnum, sizeof(FSObjInputStream *));
  for (int i = 0; i < objnum; i++) {
    string objname = filename + "_oecobj_" + to_string(i);
    createThreads[i] = thread([=] {
...
void OECWorker::readOfflineObj(string filename, string objname, int objsizeMB,
                               FSObjInputStream *objstream, int pktnum,
                               int idx) {
  ...
  if (objexist) {
    ...
    cout << "OECWorker::readOfflineObj. " << objname << " does not exist!"
         << endl;
    ...
    instreply =
        (redisReply *)redisCommand(instCtx, "blpop %s 0", instkey.c_str());
    ...
    if (opt < 0) {
      ...
      for (int computei = 0; computei < computen; computei++) {
        ...
        rReply =
            (redisReply *)redisCommand(waitCtx, "blpop %s 0", wkey.c_str());
        char *reqStr = rReply->element[1]->str;
        ECTask *compute = new ECTask(reqStr);
        compute->dump();
        freeReplyObject(rReply);
      ...
      for (int loadi = 0; loadi < loadn; loadi++) {
        ...
        readThreads[loadi] = thread([=] {
          readStreams[loadi]->readObj(ecw, curlist, _conf->_pktSize / ecw);
        });
      }

      // 2. create cache queue and cache thread
      BlockingQueue<OECDataPacket *> *writeQueue =
          new BlockingQueue<OECDataPacket *>();
      thread cacheThread = thread(
          [=] { cacheWorker(writeQueue, filename, pktnum * idx, pktnum, 1); });

      // 3. computeThread
      thread computeThread = thread([=] {
        computeWorkerDegradedOffline(readStreams, loadidx, sid2Cids, writeQueue,
                                     lostidx, computeTasks, pktnum, ecn, eck,
                                     ecw);
      });
      ...
      for (int i = 0; i < num; i++) {
        ...
        ip = ntohl(ip);
        iplist.push_back(ip);
        cout << "Fetch " << stripename << ":" << to_string(cidx) << " from "
             << RedisUtil::ip2Str(ip) << endl;
      }

      // create fetch queue
      BlockingQueue<OECDataPacket *> **fetchQueue =
          (BlockingQueue<OECDataPacket *> **)calloc(
              num, sizeof(BlockingQueue<OECDataPacket *> *));
      for (int i = 0; i < num; i++) {
        fetchQueue[i] = new BlockingQueue<OECDataPacket *>();
      }
      // create writeQueue
...
void OECWorker::readOnline(string filename, int filesizeMB, int ecn, int eck,
                           int ecw) {
  struct timeval time1, time2, time3, time4;
  gettimeofday(&time1, NULL);
  cout << "OECWorker::readOnline.filename: " << filename
       << ", filesizeMB: " << filesizeMB << ", ecn: " << ecn << ", eck: " << eck
       << ", ecw: " << ecw << endl;
  ...
  FSObjInputStream **objstreams =
      (FSObjInputStream **)calloc(ecn, sizeof(FSObjInputStream *));
  vector<thread> createThreads = vector<thread>(ecn);
  for (int i = 0; i < ecn; i++) {
    string objname = filename + "_oecobj_" + to_string(i);
    createThreads[i] = thread([=] {
  ...
  gettimeofday(&time2, NULL);
  cout << "OECWorker::readOnline.createInputStream.duration: "
       << RedisUtil::duration(time1, time2) << endl;
  ...
  if (!needRecovery) {
    ...
    // 1.1 cacheThread
    thread cacheThread =
        thread([=] { cacheWorker(writeQueue, filename, pktnum, 1); });
    ...
    cout << "OECWorker::readOnline.pushduration: "
         << RedisUtil::duration(push1, push2) << endl;
    ...
    cout << "OECWorker::readOnline.wait degraded inst = "
         << RedisUtil::duration(time1, time2) << endl;
    cout << "OECWorker::loadn = " << loadn
         << ", loadidx.size() = " << loadidx.size() << endl;

    FSObjInputStream **readStreams =
        (FSObjInputStream **)calloc(loadn, sizeof(FSObjInputStream *));
    for (int i = 0; i < loadn; i++) {
      readStreams[i] = objstreams[loadidx[i]];
      cout << "readStreams[" << i << "] = objstreams[" << loadidx[i] << "]"
           << endl;
    }
    ...
    thread computeThread = thread([=] {
      computeWorker(readStreams, loadidx, writeQueue, computeTasks, stripenum,
                    ecn, eck, ecw);
    });
  ...
  free(objstreams);
  gettimeofday(&time3, NULL);
  cout << "OECWorker::readOnline.duration: "
       << RedisUtil::duration(time1, time3) << endl;
}
...
void OECWorker::readFetchCompute(AGCommand *agCmd) {
  ...
  FSObjInputStream *objstream =
      new FSObjInputStream(_conf, readObjName, _underfs);
  if (!objstream->exist()) {
    cout << "OECWorker::readWorker." << readObjName << " does not exist!"
         << endl;
    return;
  }
  BlockingQueue<OECDataPacket *> *readQueue = objstream->getQueue();
  ...
  // create compute thread
  thread computeThread = thread([=] {
    computeWorker(fetchQueue, nprevs, prevCids, pktnum, coefs, computefor,
                  writeQueue, slicesize);
  });
  ...
  for (auto item : cacheRefs) {
    ...
    cacheThreads[cacheid++] =
        thread([=] { cacheWorker(queue, keybase, pktnum, ref); });
  }
